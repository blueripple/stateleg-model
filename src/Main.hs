...
import qualified BlueRipple.Data.Visualization.DemoCompChart as DCC
...
analyzeStatePost :: (K.KnitMany r, BRCC.CacheEffects r)
                 => BR.CommandLine
                 -> BR.PostInfo
                 -> Map Text Bool
                 -> Map Text [(GT.DistrictType, Text, Text)]
...
--    K.ignoreCacheTime statePrefByCat_C >>= K.logLE K.Info . show . MC.unPSMap
    ...
    comboByCatCacheKey <- BRCC.cacheFromDirE (MR.csProjectCacheDirE $ cacheStructureState "") ("State-Leg/dd_" <> state <> "_" <> cacheSuffix <> ".bin")
